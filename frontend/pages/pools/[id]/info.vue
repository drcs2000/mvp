    ...
    <div v-else-if="pool">
      ...
      >
        ...
        >
          <div class="flex-col">
            <h1
              class="text-xl font-bold text-gray-900 leading-tight dark:text-white"
            >
              {{ pool.name }}
            </h1>
            ...
            <div v-show="isInfoExpanded || !isMobile" class="mt-4">
              <div class="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-4">
                  ...
                  <div v-else>
                    <SwitchGroup as="div" class="flex items-center mt-1">
                      <Switch
                        ...
                        :class="
                          editForm.private ? 'bg-blue-600' : 'bg-gray-200'
                        "
                        class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 dark:bg-gray-600 dark:focus:ring-offset-gray-800"
                      >
                        <span
                          aria-hidden="true"
                          :class="
                            editForm.private ? 'translate-x-5' : 'translate-x-0'
                          "
                      ...
                      <SwitchLabel as="span" class="ml-3 text-sm">
                        <span
                          ...
                          >{{ editForm.private ? "Privado" : "Público" }}</span
                        >
                      </SwitchLabel>
                    </SwitchGroup>
                  </div>
                ...
                <div>
                  ...
                  <div v-else class="relative mt-1">
                    ...
                    >
                      <span class="text-gray-500 sm:text-sm dark:text-gray-400"
                        >R$</span
                      >
                    </div>
                    <input
                      :value="editForm.entryFee.toString().replace('.', ',')"
                      @input="handleEditEntryFeeInput"
                      type="text"
                      inputmode="decimal"
              ...
              <div v-if="isCurrentUserAdmin" class="mt-6">
                <div v-if="!isEditMode" class="flex items-center gap-4">
                  ...
                  <button
                    class="flex items-center gap-1.5 text-sm font-semibold text-blue-600 hover:text-blue-800 transition-colors duration-200 dark:text-blue-500 dark:hover:text-blue-400"
                    @click="startEditing"
                  >
                ...
                <div v-else class="flex items-center gap-4">
                  <button
                    class="px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-wait"
                    @click="saveChanges"
        ...
        <div class="space-y-2">
          ...
          >
            <div class="grid grid-cols-[auto_1fr_auto] gap-4 items-center p-4">
              <span
                class="font-semibold text-sm text-gray-400 text-center w-8 dark:text-gray-500"
                >#{{ index + 1 }}</span
              >
              <div
            ...
            <transition name="expand">
              <div v-if="isExpanded(participant.userId)" class="px-4 pb-4">
                <div
                  class="border-t border-gray-200 dark:border-gray-700 pt-4 text-xs"
                >
                  <div
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
                    <div class="flex items-center justify-between">
                      ...
                      <span
                        ...
                        >{{ participant.stats.fullHits }}</span
                      >
                    </div>
                    <div class="flex items-center justify-between">
                      <div
                      ...
                      <span
                        class="font-bold text-gray-900 dark:text-gray-100"
                        >{{ participant.stats.resultHits }}</span
                      >
                    </div>
                    <div class="flex items-center justify-between">
                      <div
                      ...
                      <span
                        class="font-bold text-gray-900 dark:text-gray-100"
                        >{{ participant.stats.partialHits }}</span
                      >
                    </div>
                    <div class="flex items-center justify-between">
                      <div
                      ...
                      <span
                        class="font-bold text-gray-900 dark:text-gray-100"
                        >{{ participant.stats.goalHits }}</span
                      >
                    </div>
                    <div
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <span
                        class="font-bold text-gray-900 dark:text-gray-100"
                        >{{ participant.stats.totalBets }}</span
                      >
                    </div>
                    <div class="flex items-center justify-between">
                      <div
        ...
        >
          ...
          <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">
            ...
            <strong class="dark:text-gray-100">{{
              participantToConfirmPayment.userName
            }}</strong>
            efetuou o pagamento da taxa de entrada?
          </p>
      ...
      >
        ...
        >
          ...
          <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">
            ...
            do bolão? Essa ação não pode ser desfeita.
          </p>
          <div class="mt-6 flex justify-center gap-3">
            <button
...
const championshipName = computed(() => {
  return pool.value?.baseChampionship?.name ?? "Campeonato";
});
...
onMounted(async () => {
  ...
  try {
    ...
    error.value = e.data?.error || e.message || "Falha ao carregar o bolão.";
  } finally {
    loading.value = false;
  }
});
...
const handleEditMaxParticipantsInput = (event) => {
  ...
  if (value > MAX_PARTICIPANTS) {
    ...
    stores.ui.showToast(
      `O número máximo de participantes é ${MAX_PARTICIPANTS}.`,
      "info"
    );
  }
...
const validateMinParticipantsOnBlur = () => {
  if (
    ...
    editForm.maxParticipants = pool.value.participants.length;
    stores.ui.showToast(
      `O mínimo de participantes é ${pool.value.participants.length} (número atual).`,
      "info"
    );
  }
};
...
const handleEditEntryFeeInput = (event) => {
  ...
  valueStr = valueStr.replace(/[^0-9,]/g, "").replace(",", ".");
  ...
  if (value > MAX_ENTRY_FEE) {
    ...
    stores.ui.showToast(
      `A taxa máxima de inscrição é de R$ ${fixedValueStr.replace(".", ",")}.`,
      "info"
    );
  } else if (value < 0) {
    value = 0;
...
async function saveChanges() {
  ...
  try {
    ...
    if (payload.maxParticipants < pool.value.participants.length) {
      stores.ui.showToast(
        `O máximo de participantes não pode ser menor que o número atual (${pool.value.participants.length}).`,
        "error"
      );
    ...
    stores.ui.showToast(
      err.data?.error || "Falha ao atualizar o bolão.",
      "error"
    );
  } finally {
    isActionLoading.value = false;
...
const confirmDeletePool = async () => {
  try {
    await stores.pools.deletePool(poolId);
    stores.ui.showToast("Bolão excluído com sucesso!", "success");
    router.push("/");
  } catch (err) {
    stores.ui.showToast(
      err.data?.error || "Falha ao excluir o bolão.",
      "error"
    );
  }
...
const confirmRemoveParticipant = async () => {
  ...
  try {
    ...
    await stores.pools.removeParticipant(poolId, userToRemoveId);
    stores.ui.showToast("Participante removido com sucesso!", "success");
    if (userToRemoveId === authStore.user?.id) {
      router.push("/");
    } else {
      pool.value.participants = pool.value.participants.filter(
        (p) => p.userId !== userToRemoveId
      );
    }
  } catch (err) {
    stores.ui.showToast(
      err.data?.error || "Falha ao remover participante.",
      "error"
    );
  } finally {
    isActionLoading.value = false;
...
const confirmPayment = async () => {
  ...
  try {
    ...
    await stores.pools.confirmPayment(poolId, userToConfirmId);
    stores.ui.showToast("Pagamento confirmado com sucesso!", "success");
    const participantInPool = pool.value.participants.find(
      (p) => p.userId === userToConfirmId
    );
    if (participantInPool) participantInPool.paid = true;
  } catch (err) {
    stores.ui.showToast(
      err.data?.error || "Falha ao confirmar pagamento.",
      "error"
    );
  } finally {
    isActionLoading.value = false;
