...
def process_data_for_training(raw_data):
    ...
    h2h_lookup = {
        f"{min(h2h['team1EspnId'], h2h['team2EspnId'])}_{max(h2h['team1EspnId'], h2h['team2EspnId'])}": h2h['matches'] for h2h in h2h_data}
    features_list, targets_list, standings = [], [], {}
    print(f"Processando {len(all_matches)} jogos para extração de features...")

    for i, match in enumerate(all_matches):
        if (i + 1) % 100 == 0:
            print(f"  -> Processando jogo {i+1}/{len(all_matches)}...")

        champ_id = match['championship']['id']
        match_date_dt = datetime.fromisoformat(
            match['date'].replace('Z', '+00:00'))
        home_id, away_id = match['homeTeamEspnId'], match['awayTeamEspnId']

        if champ_id not in standings:
            standings[champ_id] = {}
        if home_id not in standings[champ_id]:
            standings[champ_id][home_id] = {'points': 0, 'played': 0}
        if away_id not in standings[champ_id]:
            standings[champ_id][away_id] = {'points': 0, 'played': 0}

        current_standings = sorted(standings[champ_id].items(
        ), key=lambda item: item[1]['points'], reverse=True)
        ranks = {team_id: rank + 1 for rank,
                 (team_id, _) in enumerate(current_standings)}
        home_pos, away_pos = ranks.get(home_id, len(
            ranks) + 1), ranks.get(away_id, len(ranks) + 1)

        past_matches = all_matches[:i]
        home_form_matches = [m for m in reversed(past_matches) if home_id in (
            m['homeTeamEspnId'], m['awayTeamEspnId'])][:5]
        away_form_matches = [m for m in reversed(past_matches) if away_id in (
            m['homeTeamEspnId'], m['awayTeamEspnId'])][:5]
        ...
        home_form_score = calculate_weighted_form_score(
            home_form_matches, home_id)
        away_form_score = calculate_weighted_form_score(
            away_form_matches, away_id)

        h2h_key = f"{min(home_id, away_id)}_{max(home_id, away_id)}"
        h2h_matches_all = h2h_lookup.get(h2h_key, [])
        h2h_at_time = [m for m in h2h_matches_all if datetime.fromisoformat(
            m['date'].replace('Z', '+00:00')) < match_date_dt]
        ...
        if 'points' in standings[champ_id][home_id]:
            ...
            if target == 1:
                standings[champ_id][home_id]['points'] += 3
            elif target == 2:
                standings[champ_id][away_id]['points'] += 3
            else:
...
def main():
    ...
    if not raw_data or not raw_data['matches']:
        return
